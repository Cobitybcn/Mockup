...
$viewStmt=Database::connection()->prepare('SELECT file_name FROM root_artwork_candidates WHERE artwork_id=(SELECT canonical_artwork_id FROM artwork_sheets WHERE id=? AND user_id=? LIMIT 1) AND user_id=?');
$viewStmt->execute([(int)$p['artwork_sheet_id'],(int)$p['user_id'],(int)$p['user_id']]);
