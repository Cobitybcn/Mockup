...
sm_test(str_contains($managerSource, "document.addEventListener('error'") && str_contains($managerSource, "card.classList.add('visual-card--text')"), 'Broken visual thumbnails fall back to the editorial text card instead of showing a browser error icon');
...
sm_test(str_contains($managerSource, 'Open Series workspace') && str_contains($managerSource, 'Content source</dt><dd>Series Metadata') && !str_contains($managerSource, '#series-website'), 'Website Sync links to the canonical Series workspace without targeting the retired panel');
...
sm_test(!str_contains($seriesSource, 'id="series-website"') && !str_contains($seriesSource, 'class="series-website-panel"'), 'Series omits the retired Website strip and keeps one canonical metadata workspace');
