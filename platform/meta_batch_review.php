...
$backLabel = $linkedCampaign ? t('← Social campaign', '← Campaña social') : t('← Mockup Album', '← Álbum de mockups');
...
try {
    ...
    if ($hasFacebook) {
        ...
        if (app_env('META_LIVE_PUBLISH_ENABLED', 'false') !== 'true') {
            $blockers[] = t('Live Facebook publishing is disabled by environment policy.', 'La publicación en vivo de Facebook está desactivada por política del entorno.');
        }
        if (app_env('META_DRAFT_PUBLIC_MEDIA_ENABLED', 'false') !== 'true') {
            $blockers[] = t('The public Facebook media route is disabled.', 'La ruta pública de medios de Facebook está desactivada.');
        }
    }
    if ($hasInstagram) {
        try {
        ...
        if (app_env('INSTAGRAM_LIVE_PUBLISH_ENABLED', 'false') !== 'true') {
            $blockers[] = t('Live Instagram publishing is disabled by environment policy.', 'La publicación en vivo de Instagram está desactivada por política del entorno.');
        }
        if (app_env('INSTAGRAM_DRAFT_PUBLIC_MEDIA_ENABLED', 'false') !== 'true') {
            $blockers[] = t('The public Instagram media route is disabled.', 'La ruta pública de medios de Instagram está desactivada.');
        }
    }
    $publicBase = rtrim(app_env('APP_PUBLIC_URL', ''), '/');
    if (!str_starts_with(strtolower($publicBase), 'https://')) {
        $blockers[] = t('APP_PUBLIC_URL must be a public HTTPS address.', 'APP_PUBLIC_URL debe ser una dirección pública HTTPS.');
    }
    foreach ($items as $item) {
        foreach ($drafts->readiness($item) as $itemBlocker) {
            $blockers[] = sprintf(t('%s draft #%d: %s', 'Borrador de %s #%d: %s'), ucfirst((string)$item['channel']), (int)$item['id'], $itemBlocker);
        }
    }
} catch (Throwable $e) {
    $error = $e->getMessage();
...
?>
<!doctype html>
<html lang="<?= mbh(Translator::locale($user)) ?>">
<head>
    <meta charset="utf-8">
    <meta name="viewport" content="width=device-width,initial-scale=1">
    <title><?= mbh(t('Meta Batch Review', 'Revisión de lote de Meta')) ?></title>
...
<main class="main-area"><div class="workspace">
    <div class="workspace-header"><div><h1><?= mbh(t('Meta batch', 'Lote de Meta')) ?> #<?= $batchId ?></h1><p><?= count($items) ?> <?= mbh(t('Facebook / Instagram drafts', 'borradores de Facebook / Instagram')) ?><?php if ($linkedCampaign): ?> · <?= mbh($linkedCampaign['title']) ?><?php endif; ?></p></div><a class="button-link secondary" href="<?= mbh($backHref) ?>"><?= mbh($backLabel) ?></a></div>
    ...
    <?php if ($batch && (string)$batch['status'] === 'published'): ?>
        <div class="meta-publish-success" role="status" aria-live="polite">
            <strong><?= mbh(t('Published successfully on Meta.', 'Publicado correctamente en Meta.')) ?></strong>
            <span><?= mbh(t('The approved Facebook and Instagram publications are now live.', 'Las publicaciones aprobadas de Facebook e Instagram ya están activas.')) ?></span>
            <?php foreach ($items as $publishedItem): if (trim((string)$publishedItem['external_url']) !== ''): ?>
                <a href="<?= mbh($publishedItem['external_url']) ?>" target="_blank" rel="noopener"><?= mbh(t('Open publication', 'Abrir publicación')) ?></a>
            <?php break; endif; endforeach; ?>
        </div>
    <?php endif; ?>
    <?php if ($batch): ?>
        <section class="meta-summary">
            <article><strong><?= mbh(t('Identity', 'Identidad')) ?></strong><p><?= mbh($batch['purpose']) ?></p></article>
            <article><strong><?= mbh(t('Facebook Page', 'Página de Facebook')) ?></strong><p><?= mbh($metaConnection['page_name'] ?? t('Not connected', 'No conectada')) ?></p></article>
            <article><strong><?= mbh(t('Instagram', 'Instagram')) ?></strong><p><?= ($instagramConnection['username'] ?? '') !== '' ? '@' . mbh($instagramConnection['username']) : mbh(t('Not connected', 'No conectada')) ?></p></article>
            <article><strong><?= mbh(t('Batch status', 'Estado del lote')) ?></strong><p><?= mbh($batch['status']) ?></p></article>
        </section>
        <p><a class="button-link secondary" href="connections.php"><?= mbh(t('Manage connections', 'Gestionar conexiones')) ?></a></p>
        <div class="meta-grid">
        ...
        ?>
            <article class="meta-card <?= $published ? 'published-card' : '' ?>">
                ...
                <form class="meta-crop autosave">
                    ...
                    <label><?= mbh(t('Zoom', 'Zoom')) ?> <input name="crop_zoom" type="range" min="1" max="3" step=".01" value="<?= mbh($item['crop_zoom']) ?>" <?= $published ? 'disabled' : '' ?>></label>
                </form>
                <form class="meta-fields autosave">
                    ...
                    <label><span><?= (string)$item['channel'] === 'facebook' ? mbh(t('Headline', 'Título')) : mbh(t('Hook', 'Gancho')) ?></span><input type="text" name="title" maxlength="255" value="<?= mbh($item['title']) ?>" <?= $published ? 'disabled' : '' ?>></label>
                    <label><span><?= mbh(t('Publication copy', 'Texto de la publicación')) ?></span><textarea name="description" rows="6" maxlength="5000" required <?= $published ? 'disabled' : '' ?>><?= mbh($item['description']) ?></textarea></label>
                    <label><span><?= mbh(t('Hashtags', 'Hashtags')) ?></span><textarea name="hashtags" rows="2" <?= $published ? 'disabled' : '' ?>><?= mbh($hashtags) ?></textarea></label>
                    <label><span><?= mbh(t('Alt text', 'Texto alternativo')) ?></span><textarea name="alt_text" rows="3" maxlength="1000" <?= $published ? 'disabled' : '' ?>><?= mbh($item['alt_text']) ?></textarea></label>
                    <label><span><?= mbh(t('Destination HTTPS · optional', 'Destino HTTPS · opcional')) ?></span><input type="url" name="destination_url" value="<?= mbh($item['destination_url']) ?>" <?= $published ? 'disabled' : '' ?>></label>
                </form>
                <div class="meta-status"><span><?= mbh(t('Status:', 'Estado:')) ?> <?= mbh($item['status']) ?></span><span data-save-status><?= $published ? mbh(t('Published', 'Publicado')) : mbh(t('Saved automatically', 'Guardado automáticamente')) ?></span></div>
                <?php if ((string)$item['external_url'] !== ''): ?><p class="external-link"><a href="<?= mbh($item['external_url']) ?>" target="_blank" rel="noopener"><?= mbh(t('Open publication', 'Abrir publicación')) ?></a></p><?php endif; ?>
                ...
                <?php if ((string)$item['status'] === 'needs_verification'): ?>
                    <form method="post" action="meta_batch_resolve.php" class="meta-fields">
                        ...
                        <input type="hidden" name="draft_id" value="<?= (int)$item['id'] ?>">
                        <input type="hidden" name="csrf" value="<?= mbh($_SESSION['meta_batch_csrf']) ?>">
                        <label><span><?= mbh(t('Manual result', 'Resultado manual')) ?></span><select name="decision"><option value="retry"><?= mbh(t('No post exists · allow retry', 'No existe la publicación · permitir reintento')) ?></option><option value="published"><?= mbh(t('A post exists · record it', 'La publicación existe · registrarla')) ?></option></select></label>
                        <label><span><?= mbh(t('External ID · required if published', 'ID externo · requerido si está publicado')) ?></span><input type="text" name="external_id"></label>
                        <label><span><?= mbh(t('External URL · optional', 'URL externa · opcional')) ?></span><input type="url" name="external_url"></label>
                        <label><span><?= mbh(t('Type VERIFICADO', 'Escribí VERIFICADO')) ?></span><input type="text" name="confirmation_text" autocomplete="off" required></label>
                        <button class="button-link secondary"><?= mbh(t('Record manual verification', 'Registrar verificación manual')) ?></button>
                    </form>
                <?php endif; ?>
            </article>
        <?php endforeach; ?>
        ...
        <section class="publish-panel">
            <h2><?= mbh(t('Final approval', 'Aprobación final')) ?></h2>
            <?php if ($blockers): ?>
                <p><?= mbh(t('This batch cannot publish yet:', 'Este lote todavía no se puede publicar:')) ?></p><ul class="blocker-list"><?php foreach ($blockers as $blocker): ?><li><?= mbh($blocker) ?></li><?php endforeach; ?></ul>
            <?php else: ?><p><?= mbh(t('All technical checks passed. Publishing creates real public posts.', 'Todas las verificaciones técnicas pasaron. Publicar crea posteos públicos reales.')) ?></p><?php endif; ?>
            <form id="publish-meta-batch" method="post" action="meta_batch_publish.php" class="confirm-row">
                ...
                <label><input type="checkbox" name="confirm" value="yes" required> <?= mbh(t('I reviewed every Facebook and Instagram item.', 'Revisé cada elemento de Facebook e Instagram.')) ?></label>
                <label><?= mbh(t('Type', 'Escribí')) ?> <strong>PUBLICAR</strong><input type="text" name="confirmation_text" autocomplete="off" required></label>
                <button class="button-link primary" <?= $blockers ? 'disabled' : '' ?>><?= mbh(t('Publish approved Meta items', 'Publicar elementos aprobados de Meta')) ?></button>
            </form>
        </section>
    <?php endif; ?>
</div></main></div>
...
document.getElementById('publish-meta-batch')?.addEventListener('submit',async event=>{if(metaPending.size){event.preventDefault();const form=event.currentTarget;try{await Promise.all([...metaPending]);form.submit()}catch(error){alert(<?= json_encode(t('Correct the fields marked with an error before publishing.', 'Corregí los campos marcados con error antes de publicar.')) ?>)}}});
</script>
</body></html>
