...
try{
    ...
    if(($publication['status']??'')!=='published'||($publication['visibility']??'')==='private') throw new RuntimeException('First publish the landing page as public or unlisted so Pinterest can read the image and link.');
    ...
    if(!$selectedItem)throw new RuntimeException('Select a valid mockup.');
    ...
    if($_SERVER['REQUEST_METHOD']==='POST'&&($_POST['action']??'')==='publish'){
        if(!hash_equals((string)($_SESSION['pinterest_publish_csrf']??''),(string)($_POST['csrf']??'')))throw new RuntimeException('The confirmation session expired.');
        if(($_POST['confirm']??'')!=='yes')throw new RuntimeException('You must explicitly confirm publication.');
        $boardId=trim((string)($_POST['board_id']??'')); $allowed=false; foreach($boards as $board)if((string)($board['id']??'')===$boardId){$allowed=true;break;}
        if(!$allowed)throw new RuntimeException('The selected board is no longer available.');
...
<label>Board</label><select name="board_id" required><option value="">Select…</option><?php foreach($boards as $board):?><option value="<?=pph($board['id']??'')?>"><?=pph($board['name']??'Untitled')?></option><?php endforeach;?></select>
<label>Title</label><input name="title" maxlength="100" value="<?=pph($variant['title']??$selectedItem['title'])?>" required><label>Description</label><textarea name="description" maxlength="500"><?=pph($variant['description']??$selectedItem['description'])?></textarea>
<p><strong>Destino:</strong><br><?=pph(rtrim(app_env('APP_PUBLIC_URL'),'/').'/public_artwork.php?slug='.$publication['slug'])?></p>
<label class="confirm"><input type="checkbox" name="confirm" value="yes" required><span>I confirm that I want to publish this image now to the selected board.</span></label><button class="button-link primary" name="action" value="publish">Publish Pin</button>
