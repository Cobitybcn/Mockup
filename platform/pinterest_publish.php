...
try{
    ...
    if(($publication['status']??'')!=='published'||($publication['visibility']??'')==='private') throw new RuntimeException(t('First publish the landing page as public or unlisted so Pinterest can read the image and link.','Primero publicá la página como pública o no listada para que Pinterest pueda leer la imagen y el enlace.'));
    ...
    if(!$selectedItem)throw new RuntimeException(t('Select a valid mockup.','Seleccioná un mockup válido.'));
    ...
    if($_SERVER['REQUEST_METHOD']==='POST'&&($_POST['action']??'')==='publish'){
        if(!hash_equals((string)($_SESSION['pinterest_publish_csrf']??''),(string)($_POST['csrf']??'')))throw new RuntimeException(t('The confirmation session expired.','La sesión de confirmación expiró.'));
        if(($_POST['confirm']??'')!=='yes')throw new RuntimeException(t('You must explicitly confirm publication.','Tenés que confirmar explícitamente la publicación.'));
        $boardId=trim((string)($_POST['board_id']??'')); $allowed=false; foreach($boards as $board)if((string)($board['id']??'')===$boardId){$allowed=true;break;}
        if(!$allowed)throw new RuntimeException(t('The selected board is no longer available.','El tablero seleccionado ya no está disponible.'));
        ...
        $_SESSION['pinterest_publish_notice']=t('Pin published successfully.','Pin publicado correctamente.'); header('Location: pinterest_publish.php?id='.$publicationId.'&item_id='.$itemId); exit;
    }
...
<label><?=pph(t('Mockup','Mockup'))?></label><select name="item_id" onchange="location='pinterest_publish.php?id=<?=$publicationId?>&item_id='+this.value"><?php foreach($publication['items'] as $item):?><option value="<?=(int)$item['mockup_sheet_id']?>" <?=(int)$item['mockup_sheet_id']===$itemId?'selected':''?>><?=pph($item['title']?:t('Mockup #','Mockup #').$item['mockup_sheet_id'])?></option><?php endforeach;?></select>
<label><?=pph(t('Board','Tablero'))?></label><select name="board_id" required><option value=""><?=pph(t('Select…','Elegir…'))?></option><?php foreach($boards as $board):?><option value="<?=pph($board['id']??'')?>"><?=pph($board['name']??t('Untitled','Sin título'))?></option><?php endforeach;?></select>
<label><?=pph(t('Title','Título'))?></label><input name="title" maxlength="100" value="<?=pph($variant['title']??$selectedItem['title'])?>" required><label><?=pph(t('Description','Descripción'))?></label><textarea name="description" maxlength="500"><?=pph($variant['description']??$selectedItem['description'])?></textarea>
<p><strong><?=pph(t('Destination:','Destino:'))?></strong><br><?=pph(rtrim(app_env('APP_PUBLIC_URL'),'/').'/public_artwork.php?slug='.$publication['slug'])?></p>
<label class="confirm"><input type="checkbox" name="confirm" value="yes" required><span><?=pph(t('I confirm that I want to publish this image now to the selected board.','Confirmo que quiero publicar esta imagen ahora en el tablero seleccionado.'))?></span></label><button class="button-link primary" name="action" value="publish"><?=pph(t('Publish Pin','Publicar Pin'))?></button>
</aside></form><?php elseif(str_contains($error,'Conecta')):?><p><a class="button-link primary" href="connections.php#pinterest"><?=pph(t('Connect Pinterest','Conectar Pinterest'))?></a></p><?php endif;?></div></main></div></body></html>
