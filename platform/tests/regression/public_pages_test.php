...
function run_public_pages_regression_tests(): void {
    ...
    TestHarness::assertContains('Administrative account · @artworkmockups',$connections,'the platform Pinterest account is named explicitly');
    TestHarness::assertContains('Open administrative Pinterest connection',$connections,'the platform Pinterest action names its exact destination');
    TestHarness::assertContains('Use this connection in the Pinterest review video.',$connections,'the platform dialog identifies the review path');
    TestHarness::assertContains('do not save it as an artist app',$connections,'the official app cannot be silently assigned to the artist slot again');
    TestHarness::assertContains('Exchange code for access token',$callback,'Sandbox OAuth visibly pauses before exchanging the authorization code');
    TestHarness::assertContains('api-sandbox.pinterest.com/v5/oauth/token',$callback,'Sandbox OAuth identifies the token exchange endpoint');
    TestHarness::assertContains('Connecting <?=htmlspecialchars($oauthIdentity',$callback,'the Sandbox callback names the identity being connected');
    TestHarness::assertContains('OAuth app:</strong> App ID',$callback,'the Sandbox callback exposes the non-secret App ID for the review video');
    $socialBoard=(string)file_get_contents($root.'/social_media_board.php');
    TestHarness::assertContains("ADMIN · @artworkmockups · App ' . \$appId",$socialBoard,'the Pinterest board selector names the administrative account and App ID');
    TestHarness::assertContains('confirmá la etiqueta ADMIN/ARTISTA y el App ID',$socialBoard,'the Pinterest Sandbox notice requires an identity check before publishing');
