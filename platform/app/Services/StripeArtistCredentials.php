...
final class StripeArtistCredentials
{
    ...
    public function save(int $userId, string $secretKey, string $webhookSecret): array
    {
        ...
        if ($previousAccountId !== '' && $previousAccountId !== $accountId && $this->hasPendingOrders($userId, $previousAccountId)) {
            throw new RuntimeException('Resolve or cancel pending Stripe orders before changing accounts.');
        }
        ...
        $livemode = str_starts_with($secretKey, 'sk_live_');
        $now = date('c');
        $values = [
            'stripe', $accountId, $livemode ? 1 : 0, $chargesEnabled ? 1 : 0, $payoutsEnabled ? 1 : 0,
            $detailsSubmitted ? 1 : 0, $status, $this->encrypt($secretKey), $this->encrypt($webhookSecret), $now,
        ];
        if ($current) {
            $this->pdo->prepare('UPDATE artist_site_payment_connections SET provider=?,external_account_id=?,livemode=?,charges_enabled=?,payouts_enabled=?,details_submitted=?,connection_status=?,secret_key_encrypted=?,webhook_secret_encrypted=?,updated_at=? WHERE user_id=?')
                ->execute([...$values, $userId]);
        } else {
            $this->pdo->prepare('INSERT INTO artist_site_payment_connections (provider,external_account_id,livemode,charges_enabled,payouts_enabled,details_submitted,connection_status,secret_key_encrypted,webhook_secret_encrypted,created_at,updated_at,user_id) VALUES (?,?,?,?,?,?,?,?,?,?,?,?)')
                ->execute([...array_slice($values, 0, 9), $now, $now, $userId]);
        }
    ...
    public function disconnect(int $userId): void
    {
        ...
        if ($accountId !== '' && $this->hasPendingOrders($userId, $accountId)) {
            throw new RuntimeException('Resolve or cancel pending Stripe orders before disconnecting this account.');
        }
        $this->pdo->prepare("UPDATE artist_site_payment_connections SET external_account_id='',livemode=0,charges_enabled=0,payouts_enabled=0,details_submitted=0,connection_status='disconnected',secret_key_encrypted='',webhook_secret_encrypted='',updated_at=? WHERE user_id=? AND provider='stripe'")
            ->execute([date('c'), $userId]);
    }

    public function secretKey(int $userId): string
    {
        $row = $this->storedConnection($userId);
        return $row ? $this->decrypt((string)$row['secret_key_encrypted']) : '';
    }

    public function webhookSecret(int $userId): string
    {
        $row = $this->storedConnection($userId);
        return $row ? $this->decrypt((string)$row['webhook_secret_encrypted']) : '';
    }
    ...
    public static function validSecretKey(string $key): bool
    {
    ...
    private function key(): string
    {
        ...
        if (!is_string($key) || strlen($key) !== SODIUM_CRYPTO_SECRETBOX_KEYBYTES) {
            throw new RuntimeException('Stripe credential encryption is not configured on this server.');
        }
        return $key;
    }
}
