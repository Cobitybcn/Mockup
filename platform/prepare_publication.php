...
try {
    ...
    if ($id <= 0) throw new RuntimeException('Falta la publicación.');
    if ($_SERVER['REQUEST_METHOD'] === 'POST') {
        ...
        $_SESSION['publication_notice'] = $action === 'publish' ? 'Landing publicada.' : ($action === 'unpublish' ? 'Landing despublicada.' : 'Borrador guardado.');
        header('Location: prepare_publication.php?id=' . $id);
        exit;
    }
...
<h2>Mockups seleccionados</h2><p>El orden de selección se conserva por ID en esta primera versión. La obra puede tener tantas vistas como necesites.</p><div class="mockup-picker"><?php foreach($mockups as $m):?><label class="mockup-choice"><input type="checkbox" name="mockup_ids[]" value="<?=(int)$m['id']?>" <?=in_array((int)$m['id'],array_map('intval',$selected),true)?'checked':''?>><img src="media.php?file=<?=rawurlencode(basename((string)$m['mockup_file']))?>" alt="<?=hp($m['alt_text'] ?: $m['title'])?>"><div class="mockup-copy"><strong><?=hp($m['title'] ?: 'Mockup #'.$m['id'])?></strong><br><?=hp(mb_substr((string)$m['description'],0,90))?></div></label><?php endforeach;?></div>
...
<div class="publish-card" style="margin-top:14px"><h2>Variantes preparadas</h2><div class="channels"><?php foreach($publication['variants'] as $v):?><div class="channel"><strong><?=hp(ucwords(str_replace('_',' / ',$v['channel'])))?></strong><br><small><?=hp($v['format'])?> · <?=hp($v['status'])?></small><?php if($v['channel']==='pinterest'):?><p style="font-size:11px">Elige un tablero y confirma cada Pin antes de publicarlo.</p><?php if($publication['status']==='published'&&!empty($publication['items'])):?><a class="button-link secondary" href="pinterest_publish.php?id=<?=$id?>">Preparar Pin</a><?php else:?><small>Publica primero la landing y selecciona al menos un mockup.</small><?php endif;?><?php endif;?></div><?php endforeach;?></div></div></aside></div></form>
